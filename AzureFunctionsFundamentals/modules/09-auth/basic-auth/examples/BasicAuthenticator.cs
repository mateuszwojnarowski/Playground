...
/// <summary>
/// Looks up the password expected for a user. Real systems store salted password
/// hashes (for example with ASP.NET Core Identity's <c>PasswordHasher&lt;T&gt;</c>),
/// never plaintext. This learning sample keeps the contract small and pluggable.
/// </summary>
...
/// <summary>
/// Validates HTTP Basic credentials (RFC 7617): the client sends
/// <c>Authorization: Basic base64(username:password)</c>. Basic auth sends the
/// password on every request, so it must only be used over HTTPS.
/// </summary>
public sealed class BasicAuthenticator(ICredentialStore credentialStore)
{
    public BasicAuthResult Authenticate(string? authorizationHeader)
    {
        ...
        {
            // Compare against a dummy value so the response time does not reveal
